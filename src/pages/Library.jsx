...
const Library = ({ vault, isAdmin }) => {
  ...
  // Guests are now strictly 'false' for all member-only checks
  const isSeedling = isRegistered && (userTier === 'Seedling' || userTier === 'Hearthkeeper' || userTier === 'Steward');
          ...
          <div className="animate-in fade-in duration-700">
            ...
            <section className="mb-16 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-[#16121D] border border-zinc-800 p-8 rounded-[2.5rem] flex flex-col">
                   ...
                   <a href="https://podcasts.apple.com/ca/podcast/your-inner-advocate/id1722984987" target="_blank" rel="noreferrer" className="mt-auto py-3 bg-purple-500/10 text-purple-400 rounded-xl text-[9px] font-black uppercase text-center border border-purple-500/20">Listen on Apple</a>
                </div>
            ...
            {/* 2. THE BASECAMP (PUBLIC AFFILIATES) */}
            <div className="mb-8 flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                          ...
                          <div className="p-8 rounded-[2rem] bg-[#16121D] border border-zinc-800/50 flex flex-col">
                             ...
                             <a href={BOOKSHOP_URL} target="_blank" rel="noreferrer" className="mt-auto py-3 bg-purple-500/10 text-purple-400 rounded-xl text-[9px] font-black uppercase text-center border border-purple-500/20 flex items-center justify-center gap-2 hover:bg-purple-500/20 transition-all">Visit Shop <ExternalLink size={12} /></a>
                          </div>
                        </div>
                      </div>
                    )}
            ...
            {/* 3. THE LOOKOUT (HEARTHKEEPER LOCKED) */}
            <div className="mb-8 flex items-center gap-4">
                ...
                <div className="relative w-full max-w-2xl bg-[#16121D] border border-teal-500/30 p-10 md:p-14 rounded-[3rem] overflow-hidden shadow-2xl">
                  {!isHearthkeeper && (
                    <div className="absolute inset-0 z-20 bg-[#0A080D]/90 backdrop-blur-sm flex flex-col items-center justify-center p-8 text-center">
                      <Lock size={32} className="text-teal-400/40 mb-4" />
                      ...
                      <button 
                        ...
                        className="mt-6 px-8 py-3 bg-teal-500/10 border border-teal-500/30 rounded-2xl text-[9px] font-black uppercase tracking-[0.2em] text-teal-400 hover:bg-teal-500/20 transition-all"
                      >
                        Elevate Standing
                      </button>
                    </div>
                  ...
                  <div className="relative z-10">
                    ...
                    <button 
                      ...
                      disabled={!isHearthkeeper}
                      className={`h-16 px-10 rounded-2xl flex items-center gap-4 transition-all uppercase text-[10px] tracking-widest font-black ${isHearthkeeper ? 'bg-teal-500 text-black hover:bg-teal-400' : 'bg-zinc-800 text-zinc-600 cursor-not-allowed'}`}
                    >
          ...
          <div className="relative animate-in slide-in-from-right duration-500">
             {!isSteward && (
                <div className="absolute inset-0 z-50 bg-[#0A080D]/95 backdrop-blur-md flex flex-col items-center justify-center p-12 text-center min-h-[60vh]">
                   ...
                   <button onClick={() => navigate('/grove')} className="px-10 py-4 bg-purple-500 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-purple-500/20">Upgrade to Steward</button>
                </div>
             )}
             <Provisions 
               vault={vault} 
