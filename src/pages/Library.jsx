  ...
  return (
    <div className="min-h-screen bg-[#0A080D] text-zinc-300 p-4 sm:p-6 md:p-12 font-sans selection:bg-teal-500/30 overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {/* SECTION 1: THE STUDY - NEW VIBRANT COLORS */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-[#16121D] border border-teal-500/10 p-8 rounded-2xl md:rounded-[2.5rem] relative group hover:border-teal-500/30 transition-all flex flex-col shadow-xl">
              ...
              <div className="space-y-3 mt-4">
                <div className="flex items-center gap-3 p-3 rounded-xl bg-black/40 border border-teal-500/10 group-hover:border-teal-500/30 transition-all cursor-pointer" onClick={() => document.getElementById('dictionary').scrollIntoView({behavior: 'smooth'})}>
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-black/40 border border-teal-500/10 group-hover:border-teal-500/30 transition-all cursor-pointer" onClick={() => document.getElementById('outreach').scrollIntoView({behavior: 'smooth'})}>
                  <MessageSquare size={14} className="text-teal-500/50" />
                  <span className="text-[10px] font-bold text-zinc-300 uppercase tracking-tighter">Outreach Templates</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#16121D] border border-purple-500/10 p-8 rounded-2xl md:rounded-[2.5rem] relative group hover:border-purple-500/30 transition-all flex flex-col shadow-xl">
              ...
              <div className="space-y-3 mt-4">
                <div className="flex items-center gap-3 p-3 rounded-xl bg-black/40 border border-purple-500/10 group-hover:border-purple-500/30 transition-all cursor-pointer" onClick={() => document.getElementById('architect').scrollIntoView({behavior: 'smooth'})}>
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-black/40 border border-purple-500/10 group-hover:border-purple-500/30 transition-all cursor-pointer opacity-50">
        ...
        <section id="dictionary" className="mb-16 scroll-mt-24">
          ...
          <div className={`bg-[#110E16]/60 border border-teal-500/10 rounded-2xl md:rounded-[2.5rem] p-6 md:p-12 relative ${!isHearthkeeper && 'opacity-30 pointer-events-none grayscale-[0.5]'}`}>
        ...
        {/* SECTION 4: THE SCRIPT ARCHITECT */}
        <section id="architect" className="mb-16 scroll-mt-24 relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start relative">
            {!isSteward && (
                <div className="absolute inset-0 z-30 bg-[#0A080D]/70 backdrop-blur-[12px] rounded-2xl md:rounded-[2.5rem] flex flex-col items-center justify-center text-center p-12 border border-purple-500/20 shadow-2xl">
                    ...
                    <button onClick={() => navigate('/grove')} className="px-10 py-4 bg-purple-500 text-black text-[9px] font-black uppercase tracking-[0.2em] rounded-xl shadow-[0_10px_30px_rgba(168,85,247,0.3)]">Inquire About Standing</button>
                </div>
            )}
            
            {/* Input Panel */}
            <div className="space-y-6 bg-[#16121D] border border-purple-500/10 p-6 md:p-10 rounded-2xl md:rounded-[2.5rem] shadow-xl">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-[8px] font-black uppercase tracking-widest text-zinc-500">Original Sector</label>
                  <input type="text" value={ml.originSector} className="w-full bg-black/60 border border-zinc-800 p-4 rounded-xl text-[10px] text-white focus:border-purple-500/40 outline-none transition-all shadow-inner" onChange={(e) => setMl({...ml, originSector: e.target.value})} />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    className="w-full bg-black/60 border border-zinc-800 p-4 rounded-xl text-[10px] text-teal-400 font-black uppercase tracking-widest focus:border-teal-500/40 outline-none appearance-none cursor-pointer shadow-inner"
            ...
            {/* Preview Panel */}
            <div className="bg-[#16121D] border border-purple-500/10 p-6 md:p-10 rounded-2xl md:rounded-[2.5rem] relative flex flex-col min-h-[400px] shadow-xl">
                <div className="flex justify-between items-center mb-10">
                    ...
                    <div className="flex gap-2">
                        <button onClick={handleCopy} className={`p-4 rounded-xl transition-all shadow-lg ${copied ? 'bg-teal-500 text-black' : 'bg-zinc-800/80 text-zinc-300 hover:text-white hover:bg-zinc-700'}`}>
                            {copied ? <Check size={18} /> : <Copy size={18} />}
                        </button>
                        <button onClick={downloadPDF} className="p-4 rounded-xl bg-zinc-800/80 text-zinc-300 hover:text-white hover:bg-zinc-700 transition-all shadow-lg">
                            <ExternalLink size={18} />
                        </button>
                    </div>
                </div>
                <div ref={scriptRef} className="flex-1 space-y-4 font-serif italic text-sm text-zinc-300 leading-relaxed p-4 bg-[#0A080D] rounded-2xl border border-white/5 shadow-inner">
                    <p>"Hello <span className={ml.recipient ? "text-white font-bold not-italic" : "text-zinc-600"}>{ml.recipient || "[Name]"}</span>,</p>
                    <p>I’m currently transitioning from a decade in <span className="text-white font-bold not-italic">{ml.originSector}</span> into the <span className={ml.targetIndustry ? "text-white font-bold not-italic" : "text-zinc-600"}>{ml.targetIndustry || "[Industry]"}</span> space.</p>
                    <p>While my background is in {ml.originSector}, my core expertise lies in <span className={ml.reframedSkill ? "text-teal-400 font-black uppercase tracking-widest not-italic" : "text-zinc-600"}>{ml.reframedSkill || "[Reframed Skill]"}</span>—a skill set I’ve noticed is increasingly critical for teams like yours.</p>
                ...
                <button 
                  ...
                  disabled={isSaving}
                  className={`mt-8 w-full py-4 rounded-xl flex items-center justify-center gap-3 transition-all border shadow-lg ${
                    isSaving 
