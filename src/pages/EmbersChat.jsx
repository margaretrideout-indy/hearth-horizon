...
const EmberReactions = ({ msgId }) => {
  ...
  const reactionConfigs = [
    ...
    { type: 'heart', icon: Heart, color: 'text-rose-500', border: 'hover:border-rose-500/30', label: 'Love' },
  ];
...
const TierBadge = ({ tier }) => {
  ...
  switch (normalizedTier) {
    ...
    case 'hearth': return <div className="flex items-center gap-1.5 bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20"><Flame className="w-3 h-3 text-amber-500 animate-pulse" /> <span className="text-[8px] font-black text-amber-200 uppercase tracking-tighter">The Spark</span></div>;
...
export default function EmbersChat({ vault, isAdmin }) {
  ...
  const FIXED_STARTERS = [
    {
      ...
      content: "Welcome to the Hearth. This is our shared space to navigate the shifting winds of career migration. We're glad you've joined the glow.",
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0A080D] md:rounded-t-[2.5rem] overflow-hidden relative touch-none overscroll-none border border-white/5 shadow-2xl">
      ...
      <motion.div 
        ...
        className="flex-1 overflow-y-auto p-4 md:p-8 space-y-10 scrollbar-hide z-10 touch-pan-y"
      >
        <div className="space-y-12 pb-12">
            {[...FIXED_STARTERS, ...remotePosts].map((msg, idx) => {
                const isOwn = msg.email === vault?.email;
                const isSystem = msg.author_name === 'The Spark';
                ...
                return (
                    ...
                    >
                        <div className={`flex items-center gap-3 mb-2 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 ${isOwn ? 'flex-row-reverse' : ''}`}>
                            <span className={isSystem ? 'text-amber-500' : isOwn ? 'text-teal-500' : 'text-zinc-500'}>
                            ...
                            {!isOwn && (
                              <button 
                                ...
                                className="opacity-0 group-hover:opacity-100 transition-opacity ml-2 flex items-center gap-1.5 text-zinc-600 hover:text-teal-400 active:scale-95"
                              >
                                <Reply size={10} /> {isSystem ? 'CATCH THE SPARK' : 'ADD TO THE GLOW'}
                              </button>
                            )}
                        </div>

                        {msg.reply_to && (
                          <div className={`mb-2 px-3 py-1.5 border-l-2 text-[10px] italic bg-white/[0.01] rounded-r-lg max-w-[75%] ${
                            msg.reply_to.author === 'The Spark' ? 'border-amber-500/30 text-amber-200/40' : 'border-white/10 text-zinc-600'
                          } ${isOwn ? 'text-right ml-auto' : ''}`}>
                            <span className="font-bold mr-2 uppercase text-[8px]">{msg.reply_to.author}:</span>
                        ...
                        <div className={`max-w-[85%] p-4 rounded-2xl border transition-all ${
                            ...
                            <p className="text-sm leading-relaxed font-light font-serif">{msg.content}</p>
                        </div>
      ...
      <footer className="p-4 pb-8 md:pb-12 bg-[#0A080D]/95 backdrop-blur-xl border-t border-white/5 relative z-30">
          ...
          {replyTarget && (
            ...
            >
              <div className="flex items-center gap-3 text-xs text-zinc-400">
                <Reply size={14} className={replyTarget.author_name === 'The Spark' ? 'text-amber-500' : 'text-teal-500'} />
                <div className="flex flex-col">
                    <span className="text-[9px] uppercase font-black text-zinc-600 tracking-tighter">Adding to the glow of</span>
        ...
        <div className="relative flex items-center gap-3 max-w-4xl mx-auto">
          <input
            ...
            placeholder={replyTarget ? `Add to the glow...` : "Add to the glow..."}
            className="w-full bg-white/[0.03] border border-white/10 rounded-2xl px-6 py-4 text-sm text-white focus:outline-none focus:border-teal-500/40 transition-all placeholder:text-zinc-800 shadow-inner"
          />
          <button 
            onClick={handleSend} 
        ...
        <div className="flex flex-col items-center mt-4">
           <p className="text-[8px] text-zinc-700 italic text-center max-w-[280px] leading-relaxed uppercase tracking-[0.2em] font-bold">{TOS_TEXT}</p>
        </div>
      </footer>
    </div>
  );
