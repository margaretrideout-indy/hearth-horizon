...
const EmberReactions = ({ postId }) => {
  ...
  return (
    <div className="flex gap-2 mt-3 ml-1">
      <button onClick={() => addReaction('flame')} className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/[0.03] border border-white/5 hover:border-orange-500/30 transition-colors group">
        <Flame className={`w-3 h-3 ${reactions.flame > 0 ? 'text-orange-500' : 'text-zinc-600'} group-hover:text-orange-400`} />
        {reactions.flame > 0 && <span className="text-[10px] text-zinc-400">{reactions.flame}</span>}
      </button>
      <button onClick={() => addReaction('leaf')} className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/[0.03] border border-white/5 hover:border-green-500/30 transition-colors group">
        <Leaf className={`w-3 h-3 ${reactions.leaf > 0 ? 'text-green-500' : 'text-zinc-600'} group-hover:text-green-400`} />
        {reactions.leaf > 0 && <span className="text-[10px] text-zinc-400">{reactions.leaf}</span>}
      </button>
      <button onClick={() => addReaction('heart')} className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/[0.03] border border-white/5 hover:border-purple-500/30 transition-colors group">
        <Heart className={`w-3 h-3 ${reactions.heart > 0 ? 'text-purple-500' : 'text-zinc-600'} group-hover:text-purple-400`} />
        {reactions.heart > 0 && <span className="text-[10px] text-zinc-400">{reactions.heart}</span>}
      </button>
    </div>
  );
};
...
export default function EmbersChat() {
  ...
  // Auto-select prompt based on the week
  const currentPrompt = HEARTH_PROMPTS[getWeek(new Date()) % HEARTH_PROMPTS.length];
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0A080D] md:rounded-t-[2.5rem] md:border-x md:border-t border-white/5 overflow-hidden shadow-2xl relative min-h-0 flex-1">
      ...
      >
        {/* Starters & Dynamic Prompts */}
        {FIXED_STARTERS.map((msg) => (
          <div key={msg.id} className="flex flex-col items-start opacity-90 group">
            ...
            {/* Founders and The Hearth can also be reacted to! */}
      ...
      {/* Input Area */}
      <div className="p-4 pb-6 md:pb-10 bg-[#0A080D]/95 backdrop-blur-xl border-t border-white/5 shrink-0 z-20">
