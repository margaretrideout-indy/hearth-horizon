...
const Contact = ({ vault, onNavigate }) => {
  ...
  const isAdmin = true; // GOD MODE: ON
  ...
  return (
    <div className="relative min-h-screen bg-[#0A080F]">
      <div className="max-w-6xl mx-auto pb-40 px-4 animate-in fade-in duration-500">
      ...
      {/* --- PARENT NAVIGATION BAR (Bottom Anchored) --- */}
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-md px-4 z-50">
        <div className="bg-[#110E16]/80 backdrop-blur-xl border border-white/10 p-2 rounded-[2rem] shadow-2xl flex items-center justify-between">
          <button 
            onClick={() => onNavigate?.('volume1')}
            className="flex-1 flex flex-col items-center gap-1 py-2 text-zinc-500 hover:text-teal-400 transition-colors"
          >
            <BookOpen size={18} />
            <span className="text-[8px] font-black uppercase tracking-tighter">Volume I</span>
          </button>
          
          <div className="w-px h-8 bg-white/5" />

          <button 
            className="flex-1 flex flex-col items-center gap-1 py-2 text-teal-400"
          >
            <Compass size={18} />
            <span className="text-[8px] font-black uppercase tracking-tighter">Volume II</span>
          </button>
          ...
          <button 
            ...
            className="flex-1 flex flex-col items-center gap-1 py-2 text-zinc-500 hover:text-purple-400 transition-colors"
          >
            <Shield size={18} />
            <span className="text-[8px] font-black uppercase tracking-tighter">The Vault</span>
          </button>
        </div>
      </div>
    </div>
