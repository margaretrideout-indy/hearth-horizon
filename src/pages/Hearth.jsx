  ...
  return (
    <div className="min-h-screen bg-[#0F0A15] text-white font-sans selection:bg-teal-500/30 pb-20">
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {/* ZONE 2: THE EARTH (ALIGNMENT ENGINE) */}
                ...
                <div className="animate-in fade-in zoom-in duration-500 flex flex-col items-center">
                  ...
                  <div className="flex items-center gap-6">
                    <Button 
                      ...
                      className="bg-teal-500 hover:bg-teal-400 text-black font-black uppercase tracking-[0.2em] text-[11px] h-14 px-12 rounded-2xl shadow-lg shadow-teal-500/20 group"
                    >
        ...
        {/* ZONE 3: THE HEART (DAILY ROOTWORK) */}
        <div className="grid grid-cols-12 gap-12 items-start">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {pulses.length > 0 ? pulses.map((entry, i) => (
                  <Card key={i} className={`p-6 border border-white/5 rounded-2xl ${entry.reflection.includes('SYNTACTIC ALIGNMENT') ? 'bg-teal-500/5 border-teal-500/20' : 'bg-[#1C1622]/60'}`}>
                    <div className="flex justify-between items-start mb-4">
                      <div className="space-y-1">
                        <span className="text-[10px] font-bold text-slate-500 uppercase block">{new Date(entry.timestamp).toLocaleDateString()}</span>
                        <div className="flex items-center gap-1.5 text-slate-600">
                           <Clock size={10} />
                           <span className="text-[9px] font-medium uppercase">{new Date(entry.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase italic border ${entry.reflection.includes('SYNTACTIC ALIGNMENT') ? 'bg-teal-500/20 text-teal-400 border-teal-500/40' : 'bg-white/5 text-slate-400 border-white/10'}`}>
                        {entry.emoji}
                      </span>
                    </div>
          ...
          <aside className="col-span-12 lg:col-span-5 space-y-8">
            ...
            {showPreview && !isUnlocked && (
              <Card className="bg-gradient-to-br from-teal-500/10 to-transparent border-teal-500/20 p-8 rounded-[3rem] animate-in fade-in slide-in-from-bottom-4 duration-1000">
                <div className="flex items-center gap-3 mb-6"><Zap size={16} className="text-teal-400 fill-teal-400" /><p className="text-[11px] font-black uppercase text-teal-400 tracking-[0.3em]">Translation Preview</p></div>
                <div className="space-y-4">
                  <div className="p-5 bg-black/20 rounded-2xl border border-white/5">
                    <p className="text-[9px] text-slate-500 uppercase mb-1 font-bold">Classroom Management</p>
                    <p className="text-sm text-white italic">→ Operations & Logistics</p>
                  </div>
                  <div className="p-5 bg-black/20 rounded-2xl border border-white/5">
                    <p className="text-[9px] text-slate-500 uppercase mb-1 font-bold">Curriculum Design</p>
                    <p className="text-sm text-white italic">→ Program Architecture</p>
                  </div>
                </div>
              </Card>
            )}
            ...
            {isUnlocked ? (
              <Card className="bg-teal-500 p-1 rounded-[3rem] shadow-[0_0_40px_rgba(20,184,166,0.3)]">
                <Button className="w-full bg-[#1C1622] hover:bg-black text-white h-24 rounded-[2.8rem] flex flex-col items-center justify-center gap-1">
                  <Sparkles size={20} className="text-teal-400 mb-1" />
                  <span className="text-[12px] font-black uppercase tracking-[0.2em]">REVEAL THE NEXUS</span>
                </Button>
              </Card>
            ) : (
