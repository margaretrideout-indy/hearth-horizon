...
export default function YourHearth({ vault, onResumeSync, onSync, onNavigateToHorizon }) {
  ...
  // Updated with clear semantic labels for better UX
  const pulseOptions = [
    { icon: "🌱", label: "Growing" },
  ...
  const handleFileChange = (e) => {
    ...
    setTimeout(() => {
      ...
      triggerToast("Legacy document etched.");
    }, 2000);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            <div className="relative z-10 space-y-8">
                  ...
                  {pulseOptions.map((p) => (
                    <button 
                      ...
                      className={`flex flex-col items-center gap-2 py-3 rounded-2xl transition-all duration-300 ${
                        ...
                        ? "bg-rose-500/10 border-rose-500/30 scale-105" 
                        : "hover:bg-white/5 border border-transparent"
                      }`}
                    >
                      <span className="text-xl">{p.icon}</span>
                      <span className={`text-[7px] font-black uppercase tracking-tighter ${selectedEmoji === p.icon ? 'text-rose-400' : 'text-zinc-600'}`}>
                        {p.label}
                      </span>
                    </button>
                  ))}
              ...
              <div className="space-y-4">
                ...
                <Textarea 
                  ...
                  className="bg-black/40 border-white/5 rounded-[1.5rem] text-xs italic placeholder:text-zinc-800 min-h-[120px] focus:border-rose-500/30 transition-all resize-none text-white p-5"
                />
              </div>
              ...
              <Button 
                ...
                className={`w-full h-14 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all duration-500 ${
                  selectedEmoji || reflection 
                  ? "bg-rose-500 text-white shadow-[0_0_30px_rgba(244,63,94,0.2)]" 
              ...
              {vault.pulses?.slice(0, 3).map((p, i) => (
                <div key={i} className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 flex gap-4 items-center animate-in slide-in-from-left duration-500">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[8px] text-zinc-700 font-bold uppercase mt-1 tracking-widest">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <Card className="p-8 bg-teal-500/5 border border-teal-500/20 rounded-[2.5rem]">
              {vault.isAligned ? (
                ...
                <div className="text-center py-6">
                  <p className="text-xs text-zinc-600 italic mb-6">No professional alignment data found.</p>
                  <Button onClick={() => navigate('/alignment')} className="bg-zinc-900 hover:bg-teal-500 text-zinc-400 hover:text-black text-[9px] font-black uppercase rounded-full px-8 h-10 border border-white/5">
                    Establish Alignment
                  </Button>
                </div>
              )}
          ...
          <Card className="bg-[#0D0B10] border-white/5 p-10 rounded-[3rem] relative shadow-2xl">
            <div className="space-y-8">
              ...
              {!vault.resume ? (
                <label className="group flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-white/10 hover:border-teal-500/40 rounded-[2.5rem] bg-black/40 cursor-pointer transition-all">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-white">Drop Résumé / CV</p>
                  <input type="file" className="hidden" onChange={handleFileChange} />
                </label>
              ) : (
                <div className={`p-8 border rounded-[2.5rem] flex items-center justify-between transition-all ${
                  vault.resume.isOptimistic ? "bg-zinc-900 border-dashed border-zinc-700 animate-pulse" : "bg-teal-500/10 border-teal-500/20"
                }`}>
                   <div className="flex items-center gap-6">
                     <div className={`p-4 rounded-2xl ${vault.resume.isOptimistic ? "bg-zinc-800" : "bg-teal-500 text-black"}`}>
                       <FileText size={24} />
                     </div>
                     <div>
                       <h4 className="text-lg font-bold text-white truncate max-w-[200px]">{vault.resume.name}</h4>
                       <p className="text-[9px] font-black uppercase text-teal-500/60 mt-1 tracking-widest">
                         {vault.resume.isOptimistic ? "Mapping..." : "Synced"}
                       </p>
                     </div>
                   </div>
                   {!vault.resume.isOptimistic && <CheckCircle2 size={28} className="text-teal-500" />}
                </div>
              )}
            </div>
          </Card>
          ...
          <button 
             disabled={!vault.resume || vault.resume.isOptimistic}
             ...
             className={`w-full p-8 rounded-[3rem] border flex items-center justify-between group transition-all duration-700 ${
               ...
               : "bg-white/5 text-zinc-700 border-white/5 cursor-not-allowed opacity-30"
             }`}
          >
            <div className="flex items-center gap-6 text-left">
              <div className="p-4 rounded-2xl bg-black/10 border border-black/10">
                <Globe size={28} />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.4em] mb-1">Calibrated</p>
