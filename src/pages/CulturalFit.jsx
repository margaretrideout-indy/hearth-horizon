...
export default function CulturalFit({ vault, onSync, userTier = "Seedling" }) {
  ...
  const ethicsOptions = [
    ...
    { id: 'balance', label: 'The Quiet Hearth', desc: 'Strict boundaries on burnout & overwork.', icon: Flame, color: 'text-amber-500' },
  ];
                  ...
                  <Card key={idx} onClick={() => setSelectedPath(path)} className={`group cursor-pointer p-6 md:p-8 bg-[#1C1622]/40 border-white/5 transition-all duration-500 rounded-[2.5rem] relative overflow-hidden ${isSelected ? 'ring-1 ring-teal-500/50 shadow-[0_0_40px_rgba(20,184,166,0.1)] border-teal-500/40' : 'hover:border-white/10 hover:bg-[#1C1622]/60'}`}>
                    <div className="space-y-6 relative z-10">
                      <div className="flex justify-between items-start">
                         <Badge className={`${isSelected ? 'bg-teal-500 text-black' : 'bg-teal-500/10 text-teal-400'} border-transparent text-[9px] font-black px-3 py-1`}>{path.fit}% POTENTIAL MATCH</Badge>
                         {canSeeData ? <TrendingUp size={16} className={path.velocity === 'High' ? 'text-teal-400 animate-pulse' : 'text-slate-700'} /> : <Target size={14} className="text-slate-800" />}
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="pt-6 border-t border-white/5 flex justify-between items-end">
                        <div className="space-y-1">
                          ...
                          <p className={`text-xl font-black italic transition-colors ${isSelected ? 'text-white' : 'text-slate-400'}`}>{canSeeData ? path.salary : "••••••••"}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
              <div className="lg:col-span-4 space-y-8">
                  ...
                  <Card className="p-8 bg-teal-500/5 border-teal-500/20 rounded-[2.5rem] relative overflow-hidden group">
                    <motion.div className="absolute inset-0 bg-gradient-to-br from-teal-500/10 to-transparent opacity-50" animate={{ opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 4, repeat: Infinity }} />
