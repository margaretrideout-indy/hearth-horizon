...
export default function AdminDashboard({ vault, onSync, isAdmin }) {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', tier: 'Seedling' });

  // Use Environment Variables for Production
  const B44_PROJECT_ID = window.BASE44_PROJECT_ID || import.meta.env.VITE_BASE44_PROJECT_ID;
  const B44_TOKEN = window.BASE44_API_TOKEN || import.meta.env.VITE_BASE44_API_TOKEN;
  ...
  const syncBase44 = async () => {
    if (!B44_PROJECT_ID || !B44_TOKEN) return;
    setLoading(true);
    try {
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewUser({ name: '', email: '', tier: 'Seedling' });
        syncBase44();
      }
    } catch (err) {
    ...
    >
      <header className="flex flex-col md:flex-row justify-between items-start gap-8 mb-16">
        ...
        <div className="flex w-full md:w-auto gap-3 items-center">
          ...
          <button onClick={syncBase44} className="w-14 h-14 bg-white/5 flex items-center justify-center rounded-2xl border border-white/5 group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-[#1C1622]/50 p-6 md:p-10 rounded-[3rem] border border-white/5 backdrop-blur-sm">
          <div className="flex justify-between items-center mb-10">
            ...
            <button onClick={() => setIsModalOpen(true)} className="h-11 flex items-center gap-3 bg-[#39FFCA] text-[#0D0B14] px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/[0.02]">
                {filteredMembers.map((m, idx) => {
                  ...
                  return (
                    <tr key={m._id || idx} className="group hover:bg-white/[0.01]">
                      <td className="py-6">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-zinc-300">{m.name || 'Unknown'}</span>
          ...
          <div className="space-y-4 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
            {intentions.map((item, idx) => (
              <div key={idx} className="bg-black/20 p-6 rounded-[2rem] border border-white/5">
                <p className="text-[9px] font-black text-zinc-700 mb-2 uppercase">{item.email || 'Anonymous'}</p>
                <p className="text-xs text-zinc-500 italic font-serif leading-relaxed">"{item.message || item.content}"</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {isModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-[#0D0B14]/95 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-[#1C1622] w-full max-w-md rounded-[3rem] p-10 border border-white/10 relative">
              ...
              <h2 className="text-3xl font-serif italic mb-2">New Dweller</h2>
              <p className="text-emerald-500/60 text-[10px] uppercase font-black tracking-widest mb-8">Add to the Root System</p>
              <form onSubmit={handleAddUser} className="space-y-6">
                <div className="text-left">
                  <label className="text-[10px] font-black uppercase text-zinc-700 ml-2">Display Name</label>
                  <input required type="text" value={newUser.name} onChange={(e) => setNewUser({...newUser, name: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-2xl py-4 px-6 text-sm outline-none focus:ring-1 focus:ring-[#39FFCA]/20" />
                </div>
                <div className="text-left">
                  <label className="text-[10px] font-black uppercase text-zinc-700 ml-2">Email Identity</label>
                  <input required type="email" value={newUser.email} onChange={(e) => setNewUser({...newUser, email: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-2xl py-4 px-6 text-sm outline-none focus:ring-1 focus:ring-[#39FFCA]/20" />
                </div>
                <button type="submit" className="w-full h-14 bg-white text-black font-black uppercase tracking-[0.2em] rounded-2xl text-[10px] hover:bg-[#39FFCA] transition-all">Seal Identity</button>
              </form>
            </motion.div>
          </motion.div>
        )}
