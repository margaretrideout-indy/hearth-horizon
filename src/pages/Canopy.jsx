...
export default function Canopy({ vault, userTier = "Seedling" }) {
  ...
  const handleAnalyze = (job) => {
    ...
    if (userTier === "Seedling") {
      if (window.confirm("Deep alignment analysis is for Founding Forest members. Return to the Grove to upgrade?")) {
        window.location.href = "/";
      }
      return;
    }
    // 2. Data Check (The Hearth)
    if (!vault.resume) {
      if (window.confirm("To analyze this role, your Hearth must be fueled with your professional data. Go to The Hearth to upload?")) {
    ...
    if (!vault.ethics || vault.ethics.length === 0) {
      if (window.confirm("Alignment requires your Ecosystem standards. Set your ethics in Cultural Fit?")) {
    ...
    setTimeout(() => {
      setAnalysisResult({
        ...
        message: "Alignment calculated based on your Hearth data and Ecosystem ethics."
      });
      setIsAnalyzing(null);
    }, 1500);
  };
    ...
    <div className="max-w-7xl mx-auto py-12 px-6 space-y-12 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 text-sm max-w-md italic">Global opportunities for every traveler. Sync with your Hearth for deep alignment.</p>
        </div>
          ...
          return (
            <Card key={job.id} className="group p-8 bg-[#1C1622]/40 border-white/5 hover:border-teal-500/30 transition-all duration-500 rounded-[2.5rem] relative overflow-hidden flex flex-col justify-between">
              ...
              <div className="mt-8 pt-6 border-t border-white/5 space-y-4 relative z-10">
                <div className="flex justify-between items-center text-lg font-black italic text-white/80">{job.salary}</div>
      ...
      <footer className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3 bg-black/20 px-4 py-2 rounded-full border border-white/5">
          ...
          <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">
            Public Scouter: Scouting Global Forest
          </p>
        </div>
