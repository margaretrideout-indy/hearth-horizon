...
const TierBadge = () => {
  return (
    <div style={{
      display: 'inline-flex', alignItems: 'center', gap: '12px', padding: '10px 20px',
      background: 'rgba(255, 255, 255, 0.02)', borderRadius: '14px',
      border: '1px solid rgba(20, 184, 166, 0.15)', marginTop: '20px'
    }}>
      <span style={{ fontSize: '1.4rem' }}>🕯️</span>
      <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'left' }}>
        <span style={{ color: '#14b8a6', fontWeight: '900', fontSize: '0.85rem', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
          FOUNDER
        </span>
...
const Dashboard = ({ user }) => (
  <div style={{ padding: '40px 20px', maxWidth: '1000px', margin: '0 auto' }}>
    <div style={{
      background: '#0a0c12',
      padding: '60px 50px', borderRadius: '35px', border: '1px solid rgba(20, 184, 166, 0.2)',
      marginBottom: '30px', textAlign: 'left'
    }}>
      <h1 style={{ fontSize: '4.8rem', margin: '0 0 10px 0', fontWeight: '900', letterSpacing: '-0.04em', lineHeight: '0.95', color: 'white' }}>
        Welcome to the Horizon
      </h1>
      <p style={{ fontSize: '1.2rem', color: '#94a3b8', maxWidth: '550px', lineHeight: '1.5', margin: '0' }}>
    ...
    <div style={{ 
      background: '#0a0c12', padding: '40px', borderRadius: '30px', 
      border: '1px solid rgba(255, 255, 255, 0.05)', textAlign: 'left' 
    }}>
      <p style={{ color: '#14b8a6', fontSize: '0.9rem', fontWeight: '900', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        Ready for your next crossing?
      </p>
      <p style={{ color: '#94a3b8', fontSize: '1rem', margin: '0 0 25px 0', lineHeight: '1.6' }}>
        Upload your CV to <b>The Bridge Builder</b>. We'll translate your educational leadership into tech-sector data narratives.
      </p>
      <button style={{ 
        background: '#14b8a6', color: '#06080a', border: 'none', padding: '14px 32px', 
        borderRadius: '12px', fontWeight: '900', cursor: 'pointer', fontSize: '1rem'
      }}>
        Start Translation →
      </button>
    </div>
  ...
  <div style={{ padding: '40px 20px', maxWidth: '1000px', margin: '0 auto' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
      {[
        ...
        <div key={i} style={{ 
          background: '#0a0c12', padding: '30px', borderRadius: '25px', 
          border: '1px solid rgba(255,255,255,0.05)', textAlign: 'left'
        }}>
          <div style={{ fontSize: '2rem', marginBottom: '15px' }}>{item.icon}</div>
          <h4 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '900' }}>{item.title}</h4>
          <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginTop: '10px' }}>Exclusive Hearthkeeper Resource</p>
        </div>
      ))}
    </div>
  </div>
...
export default function App() {
  ...
  const navStyle = {
    display: 'flex', gap: '30px', padding: '25px 50px', background: '#06080a',
    alignItems: 'center', position: 'sticky', top: 0, zIndex: 100
  };

  const navLink = (id) => ({
    background: 'none', border: 'none', 
    color: currentPage === id ? '#14b8a6' : '#94a3b8',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#06080a', color: 'white', fontFamily: '"Inter", sans-serif' }}>
      <nav style={navStyle}>
      ...
      {currentPage === 'join' && (
        <div style={{ padding: '100px 20px', textAlign: 'center' }}>
          ...
          <p style={{color: '#94a3b8'}}>Membership tiers are being cultivated.</p>
        </div>
      )}
    </div>
  );
